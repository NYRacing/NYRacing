...
void ec_can0_init(CAN_Initialization* caninit)
{
  ...
  FLEXCAN(0).CTRL1.B.BOFFREC = 0;
             /* 0: Automatic recovering from bus off state enabled;1: disable */
  ...
  FLEXCAN(0).CTRL1.B.PROPSEG = 6;
             /* Defines the length of the propagation segment in the bit time */
  for (i=0; i<NumCanMaxMB; i++)
                         /* Initialize message buffer for receive or transmit */
  {
  ...
  FLEXCAN(0).ESR1.R = CAN_ERRINT|CAN_BOFFINT;
                             /* Error interrupt flag & Bus off interrupt flag */
  FLEXCAN(0).MCR.R = CAN_NORMAL_MODE|NUM_OF_ENABLED_MB;/* Normal mode */
  FLEXCAN(0).MCR.B.HALT = 0;
                         /* FlexCAN0 attempts to synchronize with the CAN bus */
}
...
void ec_can1_init(CAN_Initialization* caninit)
{
  ...
  FLEXCAN(1).CTRL1.B.BOFFMSK = 0;
               /* 1: Bus off interrupt enabled; 0: Bus off interrupt disabled */
  FLEXCAN(1).CTRL1.B.ERRMSK = 0;
                   /* 1: Error interrupt enabled; 0: Error interrupt disabled */
  FLEXCAN(1).CTRL1.B.CLKSRC = 0;
             /* CAN engine clock source. 0: oscillator clock; 1: system clock */
  FLEXCAN(1).CTRL1.B.LPB = 0;          /* 0: Loop back disabled */
  FLEXCAN(1).CTRL1.B.SMP = 0;          /* Number of samples */
  FLEXCAN(1).CTRL1.B.BOFFREC = 0;
            /* 0: Automatic recovering from bus off state enabled; 1: disable */
  ...
  FLEXCAN(1).ESR1.R = CAN_ERRINT|CAN_BOFFINT;
                             /* Error interrupt flag & Bus off interrupt flag */
  FLEXCAN(1).MCR.R = CAN_NORMAL_MODE|NUM_OF_ENABLED_MB;/* Normal mode */
  FLEXCAN(1).MCR.B.HALT = 0;
                         /* FlexCAN1 attempts to synchronize with the CAN bus */
}
...
void ec_can2_init(CAN_Initialization* caninit)
{
  ...
  FLEXCAN(2).CTRL1.B.BOFFMSK = 0;
               /* 1: Bus off interrupt enabled; 0: Bus off interrupt disabled */
  FLEXCAN(2).CTRL1.B.ERRMSK = 0;
                   /* 1: Error interrupt enabled; 0: Error interrupt disabled */
  FLEXCAN(2).CTRL1.B.CLKSRC = 0;
             /* CAN engine clock source. 0: oscillator clock; 1: system clock */
  FLEXCAN(2).CTRL1.B.LPB = 0;          /* 0: Loop back disabled */
  FLEXCAN(2).CTRL1.B.SMP = 0;          /* Number of samples */
  FLEXCAN(2).CTRL1.B.BOFFREC = 0;
            /* 0: Automatic recovering from bus off state enabled; 1: disable */
  ...
  FLEXCAN(2).ESR1.R = CAN_ERRINT|CAN_BOFFINT;
                             /* Error interrupt flag & Bus off interrupt flag */
  FLEXCAN(2).MCR.R = CAN_NORMAL_MODE|NUM_OF_ENABLED_MB;/* Normal mode */
  FLEXCAN(2).MCR.B.HALT = 0;
                         /* FlexCAN2 attempts to synchronize with the CAN bus */
}
...
void ec_buffer_init(uint8 Module,uint8 BufNum,uint8 IdType,uint32 Id)
{
  ...
  FLEXCAN(Module).MCR.B.HALT= 0;
                          /* FlexCAN attempts to synchronize with the CAN bus */
}
