...
enablePowertrain = 1;   % If disabled no torque will be sent to the wheels from the powertrain or brakes but external torques can be introduced by the user in the 揈nable powertrain block� in the Simulink model. 
...
enableDriverControl = 1; %% If disabled the 揹river� will not track the requested speed but the user can introduce external throttle and brake inputs in the "driver block"
...
%% Parameters needed for the equations: these shouldn抰 be changed
